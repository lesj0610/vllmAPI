...
2. Start your vllmAPI instance
3. In a separate terminal, open an ngrok instance using the command `ngrok http 5000` (or the port that vllmAPI is running on)
...
> [!NOTE]
> This is not a method for exposing your vllmAPI instance to the world. If you want that, use the other two services.

To get started:
1. Set your vllmAPI ip to `0.0.0.0` otherwise you will not be able to access your instance outside your local machine.
...
5. You can now access your vllmAPI instance via `<tailscale IP>:5000` instead of localhost as long as your are connected to your tailnet.
