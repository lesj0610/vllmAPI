...
> Sharing and hosted deployment guidance must be read together with the fork's AGPL/network-source policy in [01-license-policy.md](01-license-policy.md).
...
2. Start your ZenithForge instance
3. In a separate terminal, open an ngrok instance using the command `ngrok http 5000` (or the port that ZenithForge is running on)
...
3. Start your ZenithForge instance
...
> [!NOTE]
> This is not a method for exposing your ZenithForge instance to the world. If you want that, use the other two services.

To get started:
1. Set your ZenithForge ip to `0.0.0.0` otherwise you will not be able to access your instance outside your local machine.
...
5. You can now access your ZenithForge instance via `<tailscale IP>:5000` instead of localhost as long as your are connected to your tailnet.
