...
> Sampler override behavior may be redesigned as `ZenithForge` hardens request normalization and protocol contracts.

# Overview

Clients can send sampler parameters that users are unaware of, or even not send any parameters at all!

Sampler overrides is ZenithForge's flexible proxy for adding or forcing sampler values before generation. 
# Setting up

All supported samplers are located in `sampler_overrides/sample_preset.yml`. 

> [!NOTE]
> Sampler overrides can also be switched at runtime via the `/v1/sampling/override` endpoint set. Please read the [API docs](https://github.com/lesj0610/ZenithForge/tree/main/docs/#operation/list_sampler_overrides_v1_sampling_override_list_get) for more information.
