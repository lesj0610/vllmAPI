...
def get_install_features(lib_name: str = None):
    ...
    if install_features == "amd":
        ...
        if platform.system() == "Windows":
            print(
                "ERROR: ZenithForge does not support AMD and Windows. "
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Abridged ZenithForge start script parser.",
            epilog=(
    ...
    if start_options_path.exists():
        ...
        print(
            "It looks like you're running ZenithForge for the first time. "
    ...
    if first_run or args.update_deps:
        ...
        if args.update_deps:
            print(
                f"Dependencies updated. Please run ZenithForge with `start.{script_ext}`. "
                "Exiting."
            )
            sys.exit(0)
        else:
    ...
    if do_start_options_write:
        with open("start_options.json", "w") as start_file:
            ...
            print(
                ...
                "will reinstall ZenithForge as a first-time user."
            )
    ...
    try:
        ...
        # This is not necessary to run ZenithForge, but is new user proof
        config_path = (
